    ...
    if oc_meta:
        ...
        if authors:
            ...
            for author in split_authors:
                ...
                author_ordered_name = author_full_name.split(", ")
                if author_orcid:
                    author_data = {
                        "orcid": author_orcid.group(1),
                        "full_name": f"{author_ordered_name[1]} {author_ordered_name[0]}",
                        "related_doi": doi
                    }
...
def main(target_size=500):
    gotriple_api_docs = "https://api.gotriple.eu/api/research-products?q=open%20science&size=100&resource_type=document"
    gotriple_api_datasets = "https://api.gotriple.eu/api/research-products?q=open%20science&size=100&resource_type=dataset"
    ...
    for record in tqdm(gotriple_records):
        ...
        if oc_meta_data.status_code == 200:
            ...
            if len(opencitations_record) > 0:
                ...
                oc_index_data_citing = requests.get(oc_index_api_citing).json()
                oc_index_data_cited = requests.get(oc_index_api_cited).json()
                opencitations_index = {
